...
abstract class TrainingSessionEntryDao {
  @Query(
      'SELECT id, session_id, date, scent, rating, comment, severity, response FROM training_session_entry as tse INNER JOIN training_session as ts ON ts.id = tse.session_id WHERE ts.id = :sessionId')
